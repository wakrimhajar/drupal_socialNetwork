...
class LikeDislikeVoteBuilder implements LikeDislikeVoteBuilderInterface, TrustedCallbackInterface {
  ...
  public function build($entity_type_id, $entity_id) {
    ...
    if (!$hide_vote_widget || $like_access) {
      ...
      $like_title = $this->configFactory->get('like_and_dislike.settings')->get('like_title');
      $dislike_title = $this->configFactory->get('like_and_dislike.settings')->get('dislike_title');

      $icons['like'] = [
        'count' => $likes,
        'label' => $this->t('@label', ['@label' => $like_title]),
    ...
    if (!$hide_vote_widget || $dislike_access) {
      ...
      $icons['dislike'] = [
        'count' => $dislikes,
        'label' => $this->t('@label', ['@label' => $dislike_title]),
